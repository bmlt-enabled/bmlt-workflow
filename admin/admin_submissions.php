...
class bmaw_meeting_submissions_page extends WP_List_Table
{
    ...
    public function column_id($item)
    {
        ...
        // Get actions.
        $actions = array(
            '1'   => '<a class="bmaw_approve" id="bmaw_approve_id_'.$item['id'].'" target="_blank">' . esc_html__('Approve') . '</a>',
            '2'   => '<a class="bmaw_reject" id="bmaw_reject_id_'.$item['id'].'" target="_blank">' . esc_html__('Reject') . '</a>',
            '3'   => '<a class="bmaw_quickedit" id="bmaw_quickedit_id_'.$item['id'].'" target="_blank">' . esc_html__('Quick Edit') . '</a>',
        );
