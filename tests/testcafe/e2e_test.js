...
test('Submit_New_Meeting_And_Approve_And_Verify', async t => {
    ...
    // meeting settings
    await t
    .typeText(uf.meeting_name, meeting.meeting_name);
    ...
    .navigateTo(userVariables.crouton_page)
    ...
    await t.expect(ct.meeting_name.innerText).eql(meeting.meeting_name)
    .expect(ct.location_text.innerText).eql(meeting.location_text)
    .expect(ct.phone_meeting_number.innerText).eql(meeting.phone_meeting_number)
    .expect(ct.virtual_meeting_link.innerText).eql(meeting.virtual_meeting_link)
});
