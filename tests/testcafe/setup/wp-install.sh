...
wp option --path=/var/www/html/wordpressdev add 'bmltwf_bmlt_server_address' 'http://54.153.167.239/blank_bmlt/main_server/'
wp option --path=/var/www/html/wordpressdev add 'bmltwf_bmlt_username' 'bmlt-workflow-bot'
wp option --path=/var/www/html/wordpressdev add 'bmltwf_bmlt_test_status' 'success'
wp option --path=/var/www/html/wordpressdev add 'bmltwf_bmlt_password' '{"config":{"size":"MzI=","salt":"\/5ObzNuYZ\/Y5aoYTsr0sZw==","limit_ops":"OA==","limit_mem":"NTM2ODcwOTEy","alg":"Mg==","nonce":"VukDVzDkAaex\/jfB"},"encrypted":"fertj+qRqQrs9tC+Cc32GrXGImHMfiLyAW7sV6Xojw=="}' --format=json
# create our test page
wp post create --path=/var/www/html/wordpressdev --post_type=page --post_title='testpage' --post_content='[bmltwf-meeting-update-form]' --post_status='publish' --post_name='testpage'
# Tidy up
exit